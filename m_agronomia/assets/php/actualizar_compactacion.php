...
try {
    ...
    $cols = [
        'compactacion_id','id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela','linea',
        'palma','ubicacion','presion','profundidad','labor','latitude','longitude','supervision','error_registro','check'
    ];

    $id = $input['compactacion_id'] ?? null;
    ...
    foreach ($cols as $col) {
        if (array_key_exists($col, $input)) {
            ...
            if ($col !== 'compactacion_id') $updates[] = "$col = ?";
        }
    }
    if (empty($data)) throw new Exception("No hay datos para insertar o actualizar");
    $stmtCheck = $pg->prepare("SELECT 1 FROM compactacion WHERE compactacion_id = ?");
    $stmtCheck->execute([$id]);
    $exists = (bool)$stmtCheck->fetchColumn();

    if ($exists) {
        $sql = "UPDATE compactacion SET ".implode(',', $updates)." WHERE compactacion_id = ?";
        $valuesUpdate = array_merge(array_slice($values, 1), [$id]);
        $ok = $pg->prepare($sql)->execute($valuesUpdate);
    } else {
        $sql = "INSERT INTO compactacion (".implode(',', $data).") VALUES (".implode(',', $placeholders).")";
