...
try {
    ...
    $cols = [
        'monitoreos_generales_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela','linea','palma','grupo','estado','validacion','sintoma','observacion','supervision','check'
    ];
    ...
    foreach ($cols as $col) {
        if (array_key_exists($col, $input)) {
            ...
            if ($col !== 'monitoreos_generales_id') $updates[] = "$col = ?";
        }
    }
    if (empty($data)) throw new Exception("No hay datos para insertar o actualizar");
    $stmtCheck = $pg->prepare("SELECT 1 FROM monitoreos_generales WHERE monitoreos_generales_id = ?");
    $stmtCheck->execute([$id]);
    $exists = (bool)$stmtCheck->fetchColumn();

    if ($exists) {
        $sql = "UPDATE monitoreos_generales SET ".implode(',', $updates)." WHERE monitoreos_generales_id = ?";
