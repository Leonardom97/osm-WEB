...
$sql = "SELECT * FROM monitoreo_trampas $whereSql $orderSql LIMIT $pageSize OFFSET $offset";
$stmt = $pg->prepare($sql);
$stmt->execute($params);
$datos = $stmt->fetchAll();

// Consulta total de registros filtrados
$sqlTotal = "SELECT COUNT(*) FROM monitoreo_trampas $whereSql";
$stmtTotal = $pg->prepare($sqlTotal);
$stmtTotal->execute($params);
$total = $stmtTotal->fetchColumn();

// Respuesta final en JSON
echo json_encode(['datos' => $datos, 'total' => intval($total)]);
?>