...
try {
    ...
    $cols = [
        'ct_cal_sanidad_id','id','fecha','hora','responsable','labor','colaborador','plantacion','finca','siembra','lote',
        'parcela','linea','palma','tipo_labor','estado','etapa','instar','verificacion','observaciones','latitude','longitude','supervision','check'
    ];

    $id = $input['ct_cal_sanidad_id'] ?? null;
    ...
    foreach ($cols as $col) {
        if (array_key_exists($col, $input)) {
            ...
            if ($col !== 'ct_cal_sanidad_id') $updates[] = "$col = ?";
        }
    }
    if (empty($data)) throw new Exception("No hay datos para insertar o actualizar");
    $stmtCheck = $pg->prepare("SELECT 1 FROM ct_cal_sanidad WHERE ct_cal_sanidad_id = ?");
    $stmtCheck->execute([$id]);
    $exists = (bool)$stmtCheck->fetchColumn();

    if ($exists) {
        $sql = "UPDATE ct_cal_sanidad SET ".implode(',', $updates)." WHERE ct_cal_sanidad_id = ?";
        $valuesUpdate = array_merge(array_slice($values, 1), [$id]);
        $ok = $pg->prepare($sql)->execute($valuesUpdate);
    } else {
        $sql = "INSERT INTO ct_cal_sanidad (".implode(',', $data).") VALUES (".implode(',', $placeholders).")";
