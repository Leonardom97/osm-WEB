(function () {
  // Definición de las columnas para la tabla de oficios varios en palma
  const columnas = [
    ...
    'cantidad','unidad','maquina','tractorista','nuevo_operario','error_registro','supervision'
  ];
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.oficios_varios_palma_id))) return 'pendiente';
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.oficios_varios_palma) ? window.PENDING_IDS.oficios_varios_palma : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-oficios-chk md-checkbox" data-row="${idx}"></td>`;
          ...
          if (row.error_registro === 'inactivo') {
            ...
            td.innerHTML = `
              <button class="md-btn md-btn-icon btn-inactivar"
                      title="Inactivar"
                      data-id="${row.oficios_varios_palma_id}">
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_oficios_varios_palma.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ oficios_varios_palma_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'oficios_varios_palma_id' || soloLectura) ? "readonly" : "";
        return `
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.oficios_varios_palma_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_oficios_varios_palma.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ oficios_varios_palma_id: rowData.oficios_varios_palma_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_oficios_varios_palma.php?page=1&pageSize=1&filtro_oficios_varios_palma_id=${encodeURIComponent(rowData.oficios_varios_palma_id)}`);
            ...
            const idx = allData.findIndex(r => r.oficios_varios_palma_id == original.oficios_varios_palma_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-asistencias');
    if (!pagNav) return;
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_oficios_varios_palma.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'oficios_varios_palma_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'oficios_varios_palma_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'oficios_varios_palma_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuOficios')) return;
    const btnExport = document.getElementById('exportBtnOficios');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuOficios';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-oficios-chk').forEach(chk => chk.checked = this.checked);
      });
    }
