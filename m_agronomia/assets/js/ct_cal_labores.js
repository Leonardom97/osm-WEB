(function () {
  // Definición de las columnas que se mostrarán en la tabla de labores de calibración
  const columnas = [
    ...
    'tipo_validacion','grado_enfermedad','numero_galerias','promedio','estado','producto','observación','error_registro','supervision'
  ];

  // Variables globales para almacenar los datos, el estado de la tabla y la paginación
  let allData = [], currentPage = 1, pageSize = 25, filtros = {}, ordenColumna = null, ordenAsc = true, total = 0;

  /**
   * Función asíncrona para obtener los datos desde el servidor aplicando filtros, paginación y orden.
   */
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_ct_cal_labores.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar ct_cal_labores');
    return await resp.json();
  }

  /**
   * Determina el estado de supervisión de un registro según sus datos y un conjunto de pendientes.
   */
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.ct_cal_labores_id))) return 'pendiente';
    return 'edicion';
  }
  ...
  /**
   * Renderiza la tabla principal con los datos, agregando iconos y botones según el estado de cada registro.
   */
  function renderTabla(datos) {
    const fechaCorte = localStorage.getItem("fecha_corte");
    const tbody = document.getElementById('tbody-ct-cal-labores');
    if (!tbody) return;
    // Obtiene el conjunto de pendientes desde una variable global si existe
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.ct_cal_labores) ? window.PENDING_IDS.ct_cal_labores : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      // Checkbox para seleccionar la fila
      tr.innerHTML = `<td><input type="checkbox" class="fila-labores-chk md-checkbox" data-row="${idx}"></td>`;
      columnas.forEach(col => {
        const td = document.createElement('td');
        if (col === 'supervision') {
          // Celda especial para mostrar estado de supervisión con icono
          ...
          // Celda especial para mostrar si está inactivo o botón para inactivar
      ...
      // Lógica para botones de acción según fecha de corte y estado
      ...
      if (row.error_registro === 'inactivo') {
        ...
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.ct_cal_labores_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.ct_cal_labores_id}"><i class="fa fa-pen"></i></button>`;
        btnEstado = `<button class="md-btn md-btn-icon" title="Sin fecha de corte" disabled><i class="fa fa-question-circle"></i></button>`;
      }
      // Botones de editar, visualizar y estado
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.ct_cal_labores_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    // Asigna eventos a botones de acción
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        const id = this.getAttribute("data-id");
        const registro = allData.find(row => row.ct_cal_labores_id == id);
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_ct_cal_labores.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ct_cal_labores_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'ct_cal_labores_id' || soloLectura) ? "readonly" : "";
        return `<div class="col-md-6 mb-3"><label class="form-label">${col.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</label><input type="text" class="form-control" name="${col}" value="${rowData[col] || ''}" ${readonly}></div>`;
      }).join('');
    const contCampos = document.getElementById('campos-formulario');
    if (contCampos) contCampos.innerHTML = campos;

    // --- Icono de revertir aprobación (si el endpoint existe) ---
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.ct_cal_labores_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            // Llama endpoint para quitar la aprobación
            const resp1 = await fetch('assets/php/rechazar_ct_cal_labores.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ ct_cal_labores_id: rowData.ct_cal_labores_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_ct_cal_labores.php?page=1&pageSize=1&filtro_ct_cal_labores_id=${encodeURIComponent(rowData.ct_cal_labores_id)}`);
            ...
            const idx = allData.findIndex(r => r.ct_cal_labores_id == original.ct_cal_labores_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-ct-cal-labores');
    if (!pagNav) return;
  ...
  if (formEditar) {
    formEditar.addEventListener('submit', async function(e) {
      ...
      // Solo administradores y auxiliares agronómicos pueden aprobar directo
      ...
      try {
        const resp = await fetch('assets/php/actualizar_ct_cal_labores.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(datosEditados) });
        const result = await resp.json();
        if(result.success) {
          renderPagina();
          setTimeout(() => { document.activeElement.blur(); bootstrap.Modal.getInstance(document.getElementById('modal-editar'))?.hide(); }, 100);
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_ct_cal_labores.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'ct_cal_labores_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'ct_cal_labores_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'ct_cal_labores_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuLabores')) return;
    const btnExport = document.getElementById('exportBtnLabores');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuLabores';
  ...
  /**
   * Obtiene y muestra la página actual de la tabla.
   */
  ...
  // Asigna eventos y renderiza tabla al cargar la página
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelectLabores');
    if (select) {
    ...
    const filtrosInputs = document.querySelectorAll('#tabla-ct-cal-labores thead input[data-col]');
    ...
    const btnClear = document.getElementById('clearFiltersBtnLabores');
    if (btnClear) {
    ...
    const selectAll = document.getElementById('selectAllLabores');
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-labores-chk').forEach(chk => chk.checked = this.checked);
      });
    }
    document.querySelectorAll('#tabla-ct-cal-labores thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function() {
        ...
        document.querySelectorAll('#tabla-ct-cal-labores thead .icon-sort').forEach(f=>f.classList.remove('active'));
    ...
    // Inserta estilos para el menú de exportación
    const estilos = `
