(function () {
  // Definición de las columnas para la tabla de monitoreo de trampas
  const columnas = [
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_monitoreo_trampas.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar monitoreo_trampas');
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.monitoreo_trampas_id))) {
      return 'pendiente';
    }
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    const tbody = document.getElementById('tbody-monitoreo-trampas');
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.monitoreo_trampas) ? window.PENDING_IDS.monitoreo_trampas : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-trampas-chk md-checkbox" data-row="${idx}"></td>`;
          ...
          if (row.error_registro === 'inactivo') {
            ...
            td.innerHTML = `
              <button class="md-btn md-btn-icon btn-inactivar"
                      title="Inactivar"
                      data-id="${row.monitoreo_trampas_id}">
      ...
      if (row.error_registro === 'inactivo') {
        ...
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.monitoreo_trampas_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.monitoreo_trampas_id}"><i class="fa fa-pen"></i></button>`;
      ...
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.monitoreo_trampas_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        ...
        const registro = allData.find(row => row.monitoreo_trampas_id == id);
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_monitoreo_trampas.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ monitoreo_trampas_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'monitoreo_trampas_id' || soloLectura) ? "readonly" : "";
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.monitoreo_trampas_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_monitoreo_trampas.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ monitoreo_trampas_id: rowData.monitoreo_trampas_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_monitoreo_trampas.php?page=1&pageSize=1&filtro_monitoreo_trampas_id=${encodeURIComponent(rowData.monitoreo_trampas_id)}`);
            ...
            const idx = allData.findIndex(r => r.monitoreo_trampas_id == original.monitoreo_trampas_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-monitoreo-trampas');
    if (!pagNav) return;
  ...
  if (formEditar) {
    formEditar.addEventListener('submit', async function(e) {
      ...
      try {
        const resp = await fetch('assets/php/actualizar_monitoreo_trampas.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_monitoreo_trampas.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'monitoreo_trampas_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'monitoreo_trampas_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'monitoreo_trampas_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuTrampas')) return;
    const btnExport = document.getElementById('exportBtnTrampas');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuTrampas';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelectTrampas');
    if (select) { select.addEventListener('change', function () { pageSize = parseInt(this.value, 10); currentPage = 1; renderPagina(); }); }
    const filtrosInputs = document.querySelectorAll('#tabla-monitoreo-trampas thead input[data-col]');
    filtrosInputs.forEach(input => { input.addEventListener('input', function () { filtros[this.dataset.col] = this.value; currentPage = 1; renderPagina(); }); });
    const btnClear = document.getElementById('clearFiltersBtnTrampas');
    if (btnClear) { btnClear.addEventListener('click', () => { filtrosInputs.forEach(input => input.value = ''); filtros = {}; currentPage = 1; renderPagina(); }); }
    const selectAll = document.getElementById('selectAllTrampas');
    if (selectAll) { selectAll.addEventListener('change', function() { document.querySelectorAll('.fila-trampas-chk').forEach(chk => chk.checked = this.checked); }); }
    document.querySelectorAll('#tabla-monitoreo-trampas thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function() {
        if (ordenColumna === this.dataset.col) ordenAsc = !ordenAsc;
        else { ordenColumna = this.dataset.col; ordenAsc = true; }
        document.querySelectorAll('#tabla-monitoreo-trampas thead .icon-sort').forEach(f=>f.classList.remove('active'));
