(function () {
  // Definición de las columnas que tendrá la tabla de compactación
  const columnas = [
  ...
  // Variables globales para controlar datos, paginación, filtros y orden
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_compactacion.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar compactacion');
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.compactacion_id))) {
      return 'pendiente';
    }
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    const tbody = document.getElementById('tbody-compactacion');
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.compactacion) ? window.PENDING_IDS.compactacion : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-compactacion-chk md-checkbox" data-row="${idx}"></td>`;
      ...
      if (row.error_registro === 'inactivo') {
        ...
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.compactacion_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.compactacion_id}"><i class="fa fa-pen"></i></button>`;
      ...
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.compactacion_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        ...
        const registro = allData.find(row => row.compactacion_id == id);
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_compactacion.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ compactacion_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas.map(col => {
      const readonly = (col === 'compactacion_id' || soloLectura) ? "readonly" : "";
      return `
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.compactacion_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_compactacion.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ compactacion_id: rowData.compactacion_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_compactacion.php?page=1&pageSize=1&filtro_compactacion_id=${encodeURIComponent(rowData.compactacion_id)}`);
            ...
            const idx = allData.findIndex(r => r.compactacion_id == original.compactacion_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-compactacion');
    if (!pagNav) return;
  ...
  if (formEditar) {
    formEditar.addEventListener('submit', async function(e) {
      ...
      try {
        const resp = await fetch('assets/php/actualizar_compactacion.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_compactacion.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'compactacion_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'compactacion_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'compactacion_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuCompactacion')) return;
    const btnExport = document.getElementById('exportBtnCompactacion');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuCompactacion';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelectCompactacion');
    if (select) {
    ...
    const filtrosInputs = document.querySelectorAll('#tabla-compactacion thead input[data-col]');
    ...
    const btnClear = document.getElementById('clearFiltersBtnCompactacion');
    if (btnClear) {
    ...
    const selectAll = document.getElementById('selectAllCompactacion');
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-compactacion-chk').forEach(chk => chk.checked = this.checked);
      });
    }
    document.querySelectorAll('#tabla-compactacion thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function() {
        ...
        document.querySelectorAll('#tabla-compactacion thead .icon-sort').forEach(f=>f.classList.remove('active'));
