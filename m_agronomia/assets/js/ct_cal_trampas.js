(function () {
  const columnas = [
    'ct_cal_trampas_id','fecha','hora','responsable','colaborador','plantacion',
    'finca','siembra','lote','parcela','trampa','plaga','hembra','macho','lado_a','lado_b',
    'estado_lona','estado_trampa','estado_ubicacion','estado_ventana','estado_cania','estado_melaza',
    'estado_feromona','calificación','observaciones','error_registro','supervision'
  ];
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_ct_cal_trampas.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar ct_cal_trampas');
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.ct_cal_trampas_id))) return 'pendiente';
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    const tbody = document.getElementById('tbody-ct-cal-trampas');
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.ct_cal_trampas) ? window.PENDING_IDS.ct_cal_trampas : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      ...
      columnas.forEach(col => {
        ...
        if (col === 'supervision') {
          ...
          // Igual que en cosecha_fruta: el ícono de supervisión va aquí
      ...
      // Acciones: solo editar, visualizar, estado (lock/question)
      ...
      if (row.error_registro === 'inactivo') {
        ...
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.ct_cal_trampas_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.ct_cal_trampas_id}"><i class="fa fa-pen"></i></button>`;
      ...
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.ct_cal_trampas_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        ...
        const registro = allData.find(row => row.ct_cal_trampas_id == id);
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_ct_cal_trampas.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ct_cal_trampas_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'ct_cal_trampas_id' || soloLectura) ? "readonly" : "";
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.ct_cal_trampas_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_ct_cal_trampas.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ ct_cal_trampas_id: rowData.ct_cal_trampas_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_ct_cal_trampas.php?page=1&pageSize=1&filtro_ct_cal_trampas_id=${encodeURIComponent(rowData.ct_cal_trampas_id)}`);
            ...
            const idx = allData.findIndex(r => r.ct_cal_trampas_id == original.ct_cal_trampas_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-ct-cal-trampas');
    if (!pagNav) return;
  ...
  if (formEditar) {
    formEditar.addEventListener('submit', async function(e) {
      ...
      try {
        const resp = await fetch('assets/php/actualizar_ct_cal_trampas.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(datosEditados) });
        const result = await resp.json();
        if(result.success) {
          renderPagina();
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_ct_cal_trampas.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'ct_cal_trampas_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'ct_cal_trampas_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'ct_cal_trampas_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuTrampasCal')) return;
    const btnExport = document.getElementById('exportBtnTrampasCal');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuTrampasCal';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelectTrampasCal');
    if (select) {
    ...
    const filtrosInputs = document.querySelectorAll('#tabla-ct-cal-trampas thead input[data-col]');
    ...
    const btnClear = document.getElementById('clearFiltersBtnTrampasCal');
    if (btnClear) {
    ...
    const selectAll = document.getElementById('selectAllTrampasCal');
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-trampas-chk').forEach(chk => chk.checked = this.checked);
      });
    }
    document.querySelectorAll('#tabla-ct-cal-trampas thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function () {
        ...
        document.querySelectorAll('#tabla-ct-cal-trampas thead .icon-sort').forEach(f=>f.classList.remove('active'));
