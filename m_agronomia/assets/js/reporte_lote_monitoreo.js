(function () {
  // Definición de las columnas para la tabla de reporte de lote de monitoreo
  const columnas = [
    'reporte_lote_monitoreo_id','fecha','hora','colaborador','labor','plantacion',
    'finca','siembra','lote','parcela','linea','palma','hallazgo','observacion','error_registro','supervision'
  ];
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_reporte_lote_monitoreo.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar reporte_lote_monitoreo');
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.reporte_lote_monitoreo_id))) {
      return 'pendiente';
    }
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    const tbody = document.getElementById('tbody-reporte-lote-monitoreo');
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.reporte_lote_monitoreo) ? window.PENDING_IDS.reporte_lote_monitoreo : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-reporte-chk md-checkbox" data-row="${idx}"></td>`;
          ...
          if (row.error_registro === 'inactivo') {
            ...
            td.innerHTML = `
              <button class="md-btn md-btn-icon btn-inactivar"
                      title="Inactivar"
                      data-id="${row.reporte_lote_monitoreo_id}">
      ...
      if (row.error_registro === 'inactivo') {
        ...
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.reporte_lote_monitoreo_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.reporte_lote_monitoreo_id}"><i class="fa fa-pen"></i></button>`;
      ...
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.reporte_lote_monitoreo_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        ...
        const registro = allData.find(row => row.reporte_lote_monitoreo_id == id);
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_reporte_lote_monitoreo.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ reporte_lote_monitoreo_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'reporte_lote_monitoreo_id' || soloLectura) ? "readonly" : "";
        return `
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.reporte_lote_monitoreo_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_reporte_lote_monitoreo.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ reporte_lote_monitoreo_id: rowData.reporte_lote_monitoreo_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_reporte_lote_monitoreo.php?page=1&pageSize=1&filtro_reporte_lote_monitoreo_id=${encodeURIComponent(rowData.reporte_lote_monitoreo_id)}`);
            ...
            const idx = allData.findIndex(r => r.reporte_lote_monitoreo_id == original.reporte_lote_monitoreo_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-reporte-lote-monitoreo');
    if (!pagNav) return;
  ...
  if (formEditar) {
    formEditar.addEventListener('submit', async function(e) {
      ...
      try {
        const resp = await fetch('assets/php/actualizar_reporte_lote_monitoreo.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelectReporteLote');
    if (select) {
    ...
    const filtrosInputs = document.querySelectorAll('#tabla-reporte-lote-monitoreo thead input[data-col]');
    ...
    const btnClear = document.getElementById('clearFiltersBtnReporteLote');
    if (btnClear) {
    ...
    const selectAll = document.getElementById('selectAllReporteLote');
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-reporte-chk').forEach(chk => chk.checked = this.checked);
      });
    }
    document.querySelectorAll('#tabla-reporte-lote-monitoreo thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function() {
        ...
        document.querySelectorAll('#tabla-reporte-lote-monitoreo thead .icon-sort').forEach(f=>f.classList.remove('active'));
    ...
    function exportarTodo() {
      ...
      fetch('assets/php/conexion_reporte_lote_monitoreo.php?' + params.toString())
        .then(r => r.json())
        .then(json => {
          exportarAExcelDesdeDatos(json.datos || [], columnas, 'reporte_lote_monitoreo_todo');
        });
    }

    function exportarFiltrado() {
      exportarAExcelDesdeDatos(allData, columnas, 'reporte_lote_monitoreo_filtrado');
    }
    ...
    function exportarSeleccion() {
      ...
      exportarAExcelDesdeDatos(seleccion, columnas, 'reporte_lote_monitoreo_seleccion');
    }

    function crearMenuExportar() {
      if (document.getElementById('exportMenuReporteLote')) return;
      const btnExport = document.getElementById('exportBtnReporteLote');
      if (!btnExport) return;
      const menu = document.createElement('div');
      menu.id = 'exportMenuReporteLote';
