(function () {
  // Definición de las columnas para la tabla de monitoreos generales
  const columnas = [
    'monitoreos_generales_id','fecha','hora','colaborador','plantacion','finca','siembra','lote','parcela',
    'linea','palma','grupo','estado','validacion','sintoma','observacion','error_registro','supervision'
  ];
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    if (!resp.ok) throw new Error('Error al cargar monitoreos generales');
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.monitoreos_generales_id))) {
      return 'pendiente';
    }
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.monitoreos_generales) ? window.PENDING_IDS.monitoreos_generales : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-monitoreos-chk md-checkbox" data-row="${idx}"></td>`;
          ...
          if (row.error_registro === 'inactivo') {
            ...
            td.innerHTML = `
              <button class="md-btn md-btn-icon btn-inactivar"
                      title="Inactivar"
                      data-id="${row.monitoreos_generales_id}">
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_monitoreos_generales.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ monitoreos_generales_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'monitoreos_generales_id' || soloLectura) ? "readonly" : "";
        return `
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.monitoreos_generales_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_monitoreos_generales.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ monitoreos_generales_id: rowData.monitoreos_generales_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_monitoreos_generales.php?page=1&pageSize=1&filtro_monitoreos_generales_id=${encodeURIComponent(rowData.monitoreos_generales_id)}`);
            ...
            const idx = allData.findIndex(r => r.monitoreos_generales_id == original.monitoreos_generales_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-monitoreos-generales');
    if (!pagNav) return;
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_monitoreos_generales.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'monitoreos_generales_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'monitoreos_generales_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'monitoreos_generales_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuMonitoreos')) return;
    const btnExport = document.getElementById('exportBtnMonitoreos');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuMonitoreos';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (selectAll) {
      selectAll.addEventListener('change', function() {
        document.querySelectorAll('.fila-monitoreos-chk').forEach(chk => chk.checked = this.checked);
      });
    }
