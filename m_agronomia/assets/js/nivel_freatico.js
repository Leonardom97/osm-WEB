(function () {
  // Definición de las columnas para la tabla de nivel freático
  const columnas = [
  ...
  async function fetchData(page = 1, pageSize = 25, filtros = {}, ordenCol = null, ordenAsc = true) {
    ...
    const endpoint = `assets/php/conexion_nivel_freatico.php?${params}`;
    const resp = await fetch(endpoint);
    if (!resp.ok) throw new Error('Error al cargar nivel_freatico');
  ...
  function calcularEstadoSupervision(row, pendientesSet) {
    ...
    if (row.supervision === 'pendiente' || pendientesSet.has(String(row.nivel_freatico_id))) {
      return 'pendiente';
    }
    return 'edicion';
  }
  ...
  function renderTabla(datos) {
    ...
    const tbody = document.getElementById('tbody-nivel-freatico');
    if (!tbody) return;
    const pendientesSet = (window.PENDING_IDS && window.PENDING_IDS.nivel_freatico) ? window.PENDING_IDS.nivel_freatico : new Set();
    tbody.innerHTML = '';
    datos.forEach((row, idx) => {
      const estadoSupervision = calcularEstadoSupervision(row, pendientesSet);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td><input type="checkbox" class="fila-nivel-chk md-checkbox" data-row="${idx}"></td>`;
      ...
      if (row.error_registro === 'inactivo') {
        ...
        if (fechaFila < fechaCorte) {
          ...
          btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.nivel_freatico_id}"><i class="fa fa-pen"></i></button>`;
        }
      } else {
        btnEditar = `<button class="md-btn md-btn-icon btn-editar" title="Editar" data-id="${row.nivel_freatico_id}"><i class="fa fa-pen"></i></button>`;
      ...
      const acciones = `
        ${btnEditar}
        <button class="md-btn md-btn-icon btn-visualizar" title="Visualizar" data-id="${row.nivel_freatico_id}"><i class="fa fa-eye"></i></button>
        ${btnEstado}
      `;
    ...
    tbody.querySelectorAll('.btn-editar').forEach(btn => {
      btn.onclick = function () {
        ...
        const registro = allData.find(row => row.nivel_freatico_id == id);
    ...
    tbody.querySelectorAll('.btn-inactivar').forEach(btn => {
      btn.onclick = async function () {
        ...
        try {
          const resp = await fetch('assets/php/inactivar_nivel_freatico.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ nivel_freatico_id: id })
          });
  ...
  function abrirModal(rowData, soloLectura = false) {
    const campos = columnas
      ...
      .map(col => {
        const readonly = (col === 'nivel_freatico_id' || soloLectura) ? "readonly" : "";
    ...
    if (modalFooter) {
      ...
      if (isSupervisionCheckAprobado(rowData)) {
        ...
        repeatBtn.onclick = async function () {
          if (!rowData.nivel_freatico_id) return;
          if (!confirm("¿Deseas revertir la aprobación y restaurar los datos originales?")) return;
          try {
            const resp1 = await fetch('assets/php/rechazar_nivel_freatico.php', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ nivel_freatico_id: rowData.nivel_freatico_id })
            });
            ...
            const resp2 = await fetch(`assets/php/conexion_nivel_freatico.php?page=1&pageSize=1&filtro_nivel_freatico_id=${encodeURIComponent(rowData.nivel_freatico_id)}`);
            ...
            const idx = allData.findIndex(r => r.nivel_freatico_id == original.nivel_freatico_id);
  ...
  function renderPaginacion() {
    ...
    const pagNav = document.getElementById('pagination-nivel-freatico');
    if (!pagNav) return;
  ...
  if (formEditar) {
    formEditar.addEventListener('submit', async function(e) {
      ...
      try {
        const resp = await fetch('assets/php/actualizar_nivel_freatico.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function exportarTodo() {
    ...
    fetch('assets/php/conexion_nivel_freatico.php?' + params.toString())
      .then(r => r.json())
      .then(json => {
        exportarAExcelDesdeDatos(json.datos || [], columnas, 'nivel_freatico_todo');
      });
  }
  function exportarFiltrado() {
    exportarAExcelDesdeDatos(allData, columnas, 'nivel_freatico_filtrado');
  }
  function exportarSeleccion() {
    ...
    exportarAExcelDesdeDatos(seleccion, columnas, 'nivel_freatico_seleccion');
  }
  function crearMenuExportar() {
    if (document.getElementById('exportMenuFreatico')) return;
    const btnExport = document.getElementById('exportBtnFreatico');
    if (!btnExport) return;
    const menu = document.createElement('div');
    menu.id = 'exportMenuFreatico';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const select = document.getElementById('limitSelectFreatico');
    if (select) { select.addEventListener('change', function () { pageSize = parseInt(this.value, 10); currentPage = 1; renderPagina(); }); }
    const filtrosInputs = document.querySelectorAll('#tabla-nivel-freatico thead input[data-col]');
    ...
    const btnClear = document.getElementById('clearFiltersBtnFreatico');
    if (btnClear) { btnClear.addEventListener('click', () => { filtrosInputs.forEach(input => input.value = ''); filtros = {}; currentPage = 1; renderPagina(); }); }
    const selectAll = document.getElementById('selectAllFreatico');
    if (selectAll) { selectAll.addEventListener('change', function() { document.querySelectorAll('.fila-nivel-chk').forEach(chk => chk.checked = this.checked); }); }
    document.querySelectorAll('#tabla-nivel-freatico thead .icon-sort').forEach(flecha => {
      flecha.addEventListener('click', function() {
        if (ordenColumna === this.dataset.col) ordenAsc = !ordenAsc;
        else { ordenColumna = this.dataset.col; ordenAsc = true; }
        document.querySelectorAll('#tabla-nivel-freatico thead .icon-sort').forEach(f=>f.classList.remove('active'));
